...
public final class EventUtils {
    private static final IIntegration INTEGRATION = BukkitIntegration.getIntegration();;
    ...
    public static boolean cantAttack(@Nonnull Entity player, @Nonnull Entity victim) {
        try {
            return INTEGRATION.cantAttack(player, victim);
        } catch (Throwable throwable) {
            LOGGER.error("Failed call attack entity event: [Entity: {}, Victim: {}]", player, victim);
            if (EventHelperMod.debug)
                throwable.printStackTrace();
            return true;
        }
